...
.setThumbnail()
.addField(`Clarisa | Teşekkürler`, `**Selamlar, Ben Lord Creative (Clarisa'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Alone | Prefix`, `**Clarisa Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Clarisa | Nasıl Kullanılır?`, `**Clarisa botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Clarisa | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.setFooter(`Clarisa | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!713713727794446397>`) return msg.channel.send(`> **Clarisa | Prefix**\n\n> :tools: **Sanırım beni etiketlediniz.**\n > :tools: Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`:tools: <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Clarisa Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`:tools: **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`:tools: ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`Clarisa Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`:tools: ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`Clarisa Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`:tools: ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`Clarisa Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`:tools: ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`Clarisa Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`:tools: **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`:tools: **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`:tools: **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`:tools: **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**:tools: Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send(":tools: `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`:tools: **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send(":tools: `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send(":tools: **Selam!** `" + member.user.username + "`**!kayıtol yazarak kayıt olabilirsin!**");
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send(":tools: `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
